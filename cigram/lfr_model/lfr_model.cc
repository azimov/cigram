...
void log_msg(int type, char const *msg) {
  ...
  case WARNING:
    PyObject_CallMethod(logger, "warning", "O", pystring);
    break;
...
static PyObject *generate_graph(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "idddddiiiiilbbb", &num_nodes, &average_k, &tau,
                        &tau2, &mixing_parameter, &clustering, &max_degree,
                        &overlapping_nodes, &overlap_membership, &nmin, &nmax,
                        &seed, &excess, &defect, &fixed_range))
    return NULL;
  ...
  generate_benchmark(excess, defect, num_nodes, average_k, max_degree, tau,
                     tau2, mixing_parameter, overlapping_nodes,
                     overlap_membership, nmin, nmax, fixed_range, clustering,
                     edges, member_list);
  ...
  // The list returned is a tuple for community memberships (vertex_id,
  // membership) This is best handled at the python interface
...
static PyMethodDef LFRmodelMethods[] = {
    {"generate_graph", generate_graph, METH_VARARGS,
     "generate graph with specified parameters"},
    {NULL, NULL, 0, NULL}};
...
static struct PyModuleDef moduledef = {
    PyModuleDef_HEAD_INIT,       "lfr_model",     NULL,
    sizeof(struct module_state), LFRmodelMethods, NULL,
    LFRmodel_traverse,           LFRmodel_clear,  NULL};

#define INITERROR return NULL

// module initialization
PyMODINIT_FUNC PyInit_lfr_model(void) {
