...
int build_bipartite_network(deque<deque<int> >  & member_matrix, const deque<int> & member_numbers, const deque<int> &num_seq) {
	...
	{
		set<int> first;
		for(uint i=0; i < member_numbers.size(); i++) {
    ...
    for(uint kk=0; kk<member_numbers.size(); kk++) {
	...
	for(uint run=0; run<10; run++) {
	    for(uint node_a=0; node_a < num_seq.size(); node_a++) {
	        for(uint krm=0; krm<en_out[node_a].size(); krm++) {
...
bool excess, bool defect,  deque<int> & degree_seq, deque<int> &num_seq, deque<int> &internal_degree_seq, bool fixed_range, int nmin, int nmax, double tau2) {
	...
	for (uint i=0; i < degree_seq.size(); i++) {
	...
	if (num_seq.empty()) {
		...
		if (!fixed_range && (max_degree_actual+1)>nmin) {
		    // this helps the assignment of the memberships
		    // (it assures that at least one module is big enough to host each node)
	...
	for(uint i  =overlapping_nodes; i < degree_seq.size(); i++)
...
int build_subgraph(deque<set<int> > & E, const deque<int> & nodes, const deque<int> & degrees) {
    ...
    // ----------------------------------------------------------
    deque<int> degree_list;
    for(uint kk=0; kk < degrees.size(); kk++) {
	...
	for(int run=0; run < 10; run++) {
	    for(uint node_a=0; node_a<degrees.size(); node_a++) {
	        for(uint krm=0; krm<en[node_a].size(); krm++) {

                uint random_mate = degree_list[irand(degree_list.size()-1)];
	...
	for (uint i=0; i < multiple_edge.size(); i++) {
...
int build_subgraphs(deque<set<int> > & E, const deque<deque<int> > & member_matrix, deque<deque<int> > & member_list,
	...
	uint num_nodes = degree_seq.size();
...
int connect_all_the_parts(deque<set<int> > & E, const deque<deque<int> > & member_list, const deque<deque<int> > & link_list) {
	...
	int var_mate=0;
	for(uint i=0; i < degrees.size(); i++) {
	    for(set<int>::iterator itss= en[i].begin(); itss!=en[i].end(); itss++) if(they_are_mate(i, *itss, member_list)) {
		    var_mate++;
	    }
    }
	int stopper_mate=0;
		    ...
		    for(set<int>::iterator its= en[a].begin(); its!=en[a].end(); its++) {
                if(they_are_mate(a, *its, member_list)) {
                    ...
                    while (true) {
                        ...
                        uint random_mate =  degree_list[irand(degree_list.size()-1)];
                        while (random_mate == a || (int) random_mate == b)
                            random_mate = degree_list[irand(degree_list.size()-1)];
