...
void rewire(Graph &G, std::vector<double> &theta, std::vector<double> &beta,
            std::vector<int> &component, int &num_components,
            boost::mt19937 &generator, double a,
            const std::vector<std::vector<int>> &nodeCommunities =
  ...
  int giantC =
      std::distance(componentSize.begin(),
                    max_element(componentSize.begin(), componentSize.end()));
  int n_i = 0;
  int n_j = 0;

  // store edges in sortable format
  std::vector<std::pair<int, int>> edgeSet(num_edges(G),
                                           std::make_pair(-1, -1));
  std::vector<std::pair<double, int>> edgeScore(num_edges(G),
                                                std::make_pair(0.0, -1));
  int i = 0;
  ...
  for (tie(ei, ei_end) = edges(G); ei != ei_end; ++ei) {
    ...
    edgeScore[i] =
        std::make_pair(beta[edgeSet[i].first] + beta[edgeSet[i].second], i);
    ++i;
  }
  ...
  for (int i = 0; i < num_components; ++i) {
    ...
    std::vector<double> selectionScoresj =
        selectionScores(theta[n_i], componentNodes[giantC], beta, theta, a);
  ...
  // This rewiring procedure is extremely costly, worst case a graph contains no
  // cycles in which case every edge is tested
  for (uint i = 0; i < edgeScore.size(); ++i) {
    ...
    if (nodeCommunities.size()) {
      ...
      std::set_intersection(
          nodeCommunities[n_i].begin(), nodeCommunities[n_i].end(),
    ...
    // either we find our node and the search ends or we traverse every
    // traverseable vertex
    while (!toVisit.empty()) {
      top = toVisit.top();
 ...
 */
void setFirstFreqCounter(int num, std::vector<int> &freqCounter,
                         std::vector<int> &n_degree, std::vector<double> &beta,
 ...
 * theta should be of the same length as nodes, each index corresponding to the
 * position of n
 */
Graph fillGraph(int N, int m, std::vector<double> &theta,
                std::vector<double> &beta, std::vector<int> &n_degree,
                double sigmaR, double a, boost::mt19937 &generator,
                int minDegree = 1, int formConnected = 1) {

  Graph subGraph(N);

  // No need to sample if we're building a fully dense graph, just add every
  // edge
  if ((2.0 * m) / (N * (N - 1)) == 1.0) {
    for (int i = 0; i < N; ++i) {
      for (int j = i; j < N; ++j) {
  ...
  if (m > N - 1) {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      if (getNodeDegree(i, subGraph) < minDegree) {
        ...
        std::vector<double> selectionScoresj =
            selectionScores(theta[i], available_population, beta, theta, a);

        boost::random::discrete_distribution<> dist_j(selectionScoresj);
        SampleARes sampler(available_population, selectionScoresj);
        std::vector<int> sample = sampler.getSamplesWRS(
            minDegree - getNodeDegree(i, subGraph), generator);
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (reassign > 0 && fail_count < N) {
      ++fail_count;
      //  correction for first selection, probabilities for filled nodes will
      //  now be 0
...
void assignCommunityNodes(uint K, std::vector<int> &nodes,
                          std::vector<std::vector<int>> &communities,
                          std::vector<std::vector<int>> &nodeCommunities,
                          double overlapProb, std::vector<double> &beta_k,
                          std::vector<double> &theta_k, double a,
                          boost::mt19937 &generator, double sigmaF,
                          double sigmaR, int minCommunitySize) {
  ...
  for (size_t i = 0; i < nodes.size(); ++i) {
    ...
    if (overlapProb > 0.0) {
      ...
      for (uint j = 0; j < K; ++j) {
        sumProbs +=
            beta_k[j] * (-a * fabs(fabs(theta_k[k]) - fabs(theta_k[j])));
      }
      ...
      for (uint j = 0; j < K; ++j) {
        ...
        prob = (beta_k[j] * (-a * fabs(fabs(theta_k[k]) - fabs(theta_k[j])))) /
               sumProbs;
        // chance of overlap is community selection probability, given first
        // community multiplied by p_o parameter
...
int assignCommunityEdges(uint K, int m,
                         std::vector<std::vector<int>> &communities,
  ...
  while (interCommunityEdges < m && filled.size() < K &&
         sum_of_elems > 0.0001) {
    k = dist(generator);
    // check maximal density is not reached, if it is, remove from selectable
    if (double(communityEdgeCount[k]) <
        ((communities[k].size() * (communities[k].size() - 1)) / 2.0)) {
    ...
    sum_of_elems = 0;
    for (std::vector<double>::iterator j = beta_k.begin(); j != beta_k.end();
         ++j)
      sum_of_elems += *j;
  }
...
int overlapReassignement(Graph &G, int K, int reassignEdges,
                         std::vector<double> &beta, std::vector<double> &theta,
                         double a, std::vector<double> &beta_communities,
                         std::vector<std::vector<int>> &communities,
                         std::vector<int> &communityEdgeCount,
  ...
  for (size_t k = 0; k < communityRefill.size(); ++k) {
    ...
    for (size_t i = 0; i < firstCounter.size(); i++) {
      ...
      std::vector<double> selectionScoresj = selectionScores(
          double(theta[n_i]), available_population, beta, theta, a);

      SampleARes sampler(available_population, selectionScoresj);
      std::vector<int> sample =
          sampler.getSamplesWRS(firstCounter[i], generator);
      ...
      if (reassign > 0) {
        //  correction for first selection, probabilities for filled nodes will
        //  now be 0
        setFirstFreqCounter(reassign, firstCounter, n_degree_k[k], beta_k[k],
                            generator);
        // Must restart loop
        i = -1;
      }
...
Graph generateGraph(uint N, uint K, double Density, double sigmaF,
                    double sigmaR, double a, double communitySigmaF,
                    double communitySigmaR, double ek_per, double overlapProb,
                    std::vector<double> &theta,
                    std::vector<std::vector<int>> &communities,
                    std::vector<double> &theta_communities, int formConnected,
                    int minDegree, int minCommunitySize,
  ...
  if (K == 1) {
    ...
    G = fillGraph(N, E, theta, beta, n_degree, sigmaR, a, generator, minDegree,
                  formConnected);
    return G;
  }
  ...
  assignCommunityNodes(K, nodes, communities, nodeCommunities, overlapProb,
                       beta_communities, theta_communities, a, generator,
                       communitySigmaF, communitySigmaR, minCommunitySize);
  ...
  // calculate maximal possible overlap  and number of available edges between
  // communities
  int maxBE = 0;
  ...
  for (size_t i = 0; i < nodes.size(); ++i) {
    ...
    for (size_t j = i + 1; j < nodes.size(); ++j) {
      ...
      std::vector<int> intersection;
      std::set_intersection(
          nodeCommunities[i].begin(), nodeCommunities[i].end(),
  ...
  // Communities can end up being fully dense, we store the actual number of
  // edges assigned
  betweenEdges =
      assignCommunityEdges(K, E - betweenEdges, communities, communityEdgeCount,
                           beta_communities, generator);
  ...
  for (uint k = 0; k < K; ++k) {
    ...
    Graph subGraph = fillGraph(communities[k].size(), communityEdgeCount[k],
                               theta_k[k], beta_k[k], n_degree_k[k], sigmaR, a,
                               generator, minDegree, formConnected);
  ...
  if (reassignEdges) {
    ...
    overlapReassignement(G, K, reassignEdges, beta, theta, a, beta_communities,
                         communities, communityEdgeCount, beta_k, n_degree_k,
                         n_degree, generator);
  }
  ...
  while (int(num_edges(G)) < int(E) and interCount < maxBE and
         exit_early < EXIT_EARLY_CONDITION) {
    ...
    for (uint i = 0; i < nodeFreqCounter.size(); ++i) {
      ...
      // get correct weights
      std::vector<double> selectionScoresj = interCommunitySelectionScores(
          i, theta[i], available_population, beta, theta, nodeCommunities,
          beta_communities, max_community, a);

      SampleARes sampler(available_population, selectionScoresj);
      std::vector<int> sample =
          sampler.getSamplesWRS(nodeFreqCounter[i], generator);
  ...
  if (exit_early >= EXIT_EARLY_CONDITION) {
    log_msg(WARNING, "Unable to assign edges between communities, terminated "
                     "without completion");
  }
  ...
  if (num_components > 1 && formConnected && uint(ek_per * E) >= K - 1) {
    rewire(G, theta, beta, component, num_components, generator, a,
           nodeCommunities);
  }
  return G;
}
...
/*
 * Converts graph to an adjacency list, not used but useful for testing if
 * building outside of python
 */
