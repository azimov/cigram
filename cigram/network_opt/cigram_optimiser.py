...
class CigramOptimiser(object):
    ...
    def parameter_generator(self, random, args):
        """
        Geneartes a random set of parameters for use by the optimiser
        if the starting population kwarg is used at init, the population is not randomised but starts with specified values
        """
    ...
    def get_fitness(self, candidate):
        ...
        This function tests the assortativity and distance of the degree distribution (using the KS and log difference of max degree)
    ...
    def get_fitness_avg(self, candidate, num_reps=1):
        ...
        if num_reps < 1:
                num_reps = 1
        graphs = [self.generate(candidate) for s in range(num_reps)]
